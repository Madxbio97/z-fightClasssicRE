...
#define D3D1_MAX_STATE_PATCHES 16
#define D3D1_OVERLAY_TEXTURE_CAPACITY 128
...
typedef struct D3DHVERTEX_COMPAT {
  DWORD flags;
  float hx;
  float hy;
  float hz;
...
typedef struct D3D1StatePatch {
  DWORD value_offset;
  DWORD original_value;
} D3D1StatePatch;

typedef struct D3D1ExecutePatchList {
  DWORD count;
  D3D1StatePatch patches[D3D1_MAX_STATE_PATCHES];
} D3D1ExecutePatchList;
...
static const GUID kIID_IDirect3DTexture =
...
static DWORD g_d3d1_overlay_textures[D3D1_OVERLAY_TEXTURE_CAPACITY];
...
static volatile LONG g_d3d1_overlay_skipped = 0;
static volatile LONG g_d3d1_overlay_logged = 0;
static volatile LONG g_d3d1_overlay_texture_count = 0;
static volatile LONG g_d3d1_overlay_learned = 0;
static volatile LONG g_d3d1_overlay_probe_logged = 0;
...
static const int g_re1_d3d1_skip_screen_overlay_alpha = 1;
...
static const float g_re1_d3d1_overlay_min_extent = 4.0f;
static const float g_re1_d3d1_overlay_min_area = 48.0f;
static const float g_re1_d3d1_overlay_max_extent = 4096.0f;
static const float g_re1_d3d1_overlay_h_min_extent = 0.0100f;
static const float g_re1_d3d1_overlay_h_min_area = 0.000050f;
static const float g_re1_d3d1_overlay_flat_z_span = 0.000800f;
...
static HRESULT STDMETHODCALLTYPE Hook_D3DTexture_GetHandle(void* self, void* device, DWORD* handle);
...
static void LogDrawCallsiteSummary(const char* reason)
{
  ...
  LogLine("summary re1_d3d1 devices=%ld executes=%ld patchedExecutes=%ld shadowStatePatches=%ld "
          "overlaySkipped=%ld overlayTextures=%ld overlayLearned=%ld qiLogged=%ld "
          "shadowClip=%d overlaySkip=%d",
          g_d3d1_device_seen, g_d3d1_execute_seen, g_d3d1_execute_patched,
          g_d3d1_shadow_state_patches, g_d3d1_overlay_skipped,
          g_d3d1_overlay_texture_count, g_d3d1_overlay_learned,
          g_d3d1_qi_logged, g_re1_d3d1_execute_shadow_clip,
          g_re1_d3d1_skip_screen_overlay_alpha);
  for (LONG i = 0; i < count; i++)
  {
...
static int IsPlausibleD3D1Float(float value, float limit)
{
  return value == value && value > -limit && value < limit;
}

static int IsPlausibleD3D1TLVertex(const D3DTLVERTEX_COMPAT* vertex)
{
  if (!vertex)
    return 0;
  if (!IsPlausibleD3D1Float(vertex->sx, 8192.0f) ||
      !IsPlausibleD3D1Float(vertex->sy, 8192.0f) ||
      !IsPlausibleD3D1Float(vertex->sz, 32.0f) ||
      !IsPlausibleD3D1Float(vertex->rhw, 32.0f) ||
      !IsPlausibleD3D1Float(vertex->tu, 64.0f) ||
      !IsPlausibleD3D1Float(vertex->tv, 64.0f))
    return 0;
  return 1;
}

static int IsPlausibleD3D1HVertex(const D3DHVERTEX_COMPAT* vertex)
{
  if (!vertex)
    return 0;
  if (!IsPlausibleD3D1Float(vertex->hx, 8192.0f) ||
      !IsPlausibleD3D1Float(vertex->hy, 8192.0f) ||
      !IsPlausibleD3D1Float(vertex->hz, 8192.0f))
    return 0;
  return 1;
}

static const D3DTLVERTEX_COMPAT* GetD3D1TLVertices(const D3DEXECUTEDATA_COMPAT* exec_data,
                                                   const D3DEXECUTEBUFFERDESC_COMPAT* desc,
                                                   DWORD* vertex_count)
{
  if (vertex_count)
    *vertex_count = 0;
  if (!exec_data || !desc || !desc->data || exec_data->vertex_count == 0)
    return NULL;

  const DWORD bytes = exec_data->vertex_count * (DWORD)sizeof(D3DTLVERTEX_COMPAT);
  if (exec_data->vertex_count > 4096u || bytes / sizeof(D3DTLVERTEX_COMPAT) != exec_data->vertex_count)
    return NULL;
  if (exec_data->vertex_offset >= desc->buffer_size ||
      bytes > desc->buffer_size - exec_data->vertex_offset)
    return NULL;

  const D3DTLVERTEX_COMPAT* vertices =
    (const D3DTLVERTEX_COMPAT*)((const BYTE*)desc->data + exec_data->vertex_offset);
  DWORD sample_count = exec_data->vertex_count < 8u ? exec_data->vertex_count : 8u;
  for (DWORD i = 0; i < sample_count; i++)
  {
    if (!IsPlausibleD3D1TLVertex(&vertices[i]))
      return NULL;
  }

  if (vertex_count)
    *vertex_count = exec_data->vertex_count;
  return vertices;
}

static const D3DHVERTEX_COMPAT* GetD3D1HVertices(const D3DEXECUTEDATA_COMPAT* exec_data,
                                                 const D3DEXECUTEBUFFERDESC_COMPAT* desc,
                                                 DWORD* vertex_count)
{
  if (vertex_count)
    *vertex_count = 0;
  if (!exec_data || !desc || !desc->data || exec_data->hvertex_offset >= desc->buffer_size)
    return NULL;

  DWORD available = desc->buffer_size - exec_data->hvertex_offset;
  if (exec_data->instruction_offset > exec_data->hvertex_offset)
    available = exec_data->instruction_offset - exec_data->hvertex_offset;

  DWORD count = available / (DWORD)sizeof(D3DHVERTEX_COMPAT);
  if (count > 4096u)
    count = 4096u;
  if (count == 0)
    return NULL;

  const D3DHVERTEX_COMPAT* vertices =
    (const D3DHVERTEX_COMPAT*)((const BYTE*)desc->data + exec_data->hvertex_offset);
  DWORD sample_count = count < 8u ? count : 8u;
  for (DWORD i = 0; i < sample_count; i++)
  {
    if (!IsPlausibleD3D1HVertex(&vertices[i]))
      return NULL;
  }
...
static void ExpandD3D1Bounds(DrawBounds* bounds, const D3DTLVERTEX_COMPAT* vertex, int* valid)
{
  if (!bounds || !vertex || !valid)
    return;
  ExpandBoundsWithVertex(bounds, vertex, valid);
}

static void ExpandD3D1HBounds(DrawBounds* bounds, const D3DHVERTEX_COMPAT* vertex, int* valid)
{
  if (!bounds || !vertex || !valid)
    return;
  ...
  if (!*valid)
  {
    bounds->min_x = bounds->max_x = vertex->hx;
    bounds->min_y = bounds->max_y = vertex->hy;
    bounds->min_z = bounds->max_z = vertex->hz;
    bounds->min_rhw = bounds->max_rhw = 0.0f;
    *valid = 1;
    return;
  }

  if (vertex->hx < bounds->min_x)
    bounds->min_x = vertex->hx;
  if (vertex->hx > bounds->max_x)
    bounds->max_x = vertex->hx;
  if (vertex->hy < bounds->min_y)
    bounds->min_y = vertex->hy;
  if (vertex->hy > bounds->max_y)
    bounds->max_y = vertex->hy;
  if (vertex->hz < bounds->min_z)
    bounds->min_z = vertex->hz;
  if (vertex->hz > bounds->max_z)
    bounds->max_z = vertex->hz;
}

static int ComputeD3D1TriangleBatchBounds(const D3DTLVERTEX_COMPAT* vertices, DWORD vertex_count,
                                          const D3DTRIANGLE_COMPAT* triangles, DWORD triangle_count,
                                          DrawBounds* bounds, DWORD* min_alpha, DWORD* max_alpha,
                                          WORD* unique_vertices, DWORD* unique_count)
{
  if (bounds)
    memset(bounds, 0, sizeof(*bounds));
  if (min_alpha)
  ...
  if (unique_count)
    *unique_count = 0;
  if (!vertices || !triangles || !bounds || triangle_count == 0 || vertex_count == 0)
    return 0;

  int valid = 0;
  DWORD local_min_alpha = 255u;
  DWORD local_max_alpha = 0u;
  DWORD local_unique_count = 0;
  const DWORD unique_capacity = unique_vertices ? 12u : 0u;

  for (DWORD i = 0; i < triangle_count; i++)
  {
    const WORD indices[3] = {triangles[i].v1, triangles[i].v2, triangles[i].v3};
    for (DWORD j = 0; j < 3; j++)
    {
      const DWORD idx = (DWORD)indices[j];
      if (idx >= vertex_count || !IsPlausibleD3D1TLVertex(&vertices[idx]))
        return 0;

      const D3DTLVERTEX_COMPAT* vertex = &vertices[idx];
      ExpandD3D1Bounds(bounds, vertex, &valid);
      const DWORD alpha = (vertex->color >> 24) & 0xFFu;
      if (alpha < local_min_alpha)
        local_min_alpha = alpha;
      if (alpha > local_max_alpha)
        local_max_alpha = alpha;

      int seen = 0;
      for (DWORD k = 0; k < local_unique_count; k++)
      {
        if (unique_vertices[k] == indices[j])
        {
          seen = 1;
          break;
        }
      }
      if (!seen && unique_vertices && local_unique_count < unique_capacity)
        unique_vertices[local_unique_count++] = indices[j];
    }
  }
  ...
  if (max_alpha)
    *max_alpha = local_max_alpha;
  if (unique_count)
    *unique_count = local_unique_count;
  return 1;
}

static int ComputeD3D1HTriangleBatchBounds(const D3DHVERTEX_COMPAT* vertices, DWORD vertex_count,
                                           const D3DTRIANGLE_COMPAT* triangles, DWORD triangle_count,
                                           DrawBounds* bounds, WORD* unique_vertices,
                                           DWORD* unique_count)
{
  if (bounds)
    memset(bounds, 0, sizeof(*bounds));
  ...
  for (DWORD i = 0; i < triangle_count; i++)
  {
    ...
    {
      const DWORD idx = (DWORD)indices[j];
      if (idx >= vertex_count || !IsPlausibleD3D1HVertex(&vertices[idx]))
        return 0;

      ExpandD3D1HBounds(bounds, &vertices[idx], &valid);
...
static int D3D1VerticesMatchBoundsAxis(const D3DTLVERTEX_COMPAT* vertices, const WORD* unique_vertices,
                                       DWORD unique_count, const DrawBounds* bounds)
{
  if (!vertices || !unique_vertices || !bounds || unique_count < 4)
    return 0;

  for (DWORD i = 0; i < unique_count; i++)
  {
    const D3DTLVERTEX_COMPAT* vertex = &vertices[(DWORD)unique_vertices[i]];
    if (!NearF(vertex->sx, bounds->min_x, g_re1_d3d1_overlay_axis_epsilon) &&
        !NearF(vertex->sx, bounds->max_x, g_re1_d3d1_overlay_axis_epsilon))
      return 0;
    if (!NearF(vertex->sy, bounds->min_y, g_re1_d3d1_overlay_axis_epsilon) &&
        !NearF(vertex->sy, bounds->max_y, g_re1_d3d1_overlay_axis_epsilon))
      return 0;
  }
  return 1;
}

static int IsLikelyD3D1ScreenOverlayAlpha(const D3DTLVERTEX_COMPAT* vertices, DWORD vertex_count,
                                          const D3DHVERTEX_COMPAT* hvertices, DWORD hvertex_count,
                                          const D3DTRIANGLE_COMPAT* triangles, DWORD triangle_count,
                                          DrawBounds* out_bounds, DWORD* out_min_alpha,
                                          DWORD* out_max_alpha)
{
  if (!g_re1_d3d1_skip_screen_overlay_alpha || triangle_count != 2)
    return 0;

  DrawBounds bounds;
  WORD unique_vertices[12];
  DWORD unique_count = 0;
  DWORD min_alpha = 255u;
  DWORD max_alpha = 0u;
  if (ComputeD3D1TriangleBatchBounds(vertices, vertex_count, triangles, triangle_count,
                                     ...
                                     unique_vertices, &unique_count))
  {
    ...
    const float height = AbsF(bounds.height);
    const float extent = width > height ? width : height;
    if (width >= g_re1_d3d1_overlay_min_extent &&
        height >= g_re1_d3d1_overlay_min_extent &&
        bounds.area >= g_re1_d3d1_overlay_min_area &&
        extent <= g_re1_d3d1_overlay_max_extent &&
        AbsF(bounds.max_z - bounds.min_z) <= g_re1_d3d1_overlay_flat_z_span &&
        AbsF(bounds.max_rhw - bounds.min_rhw) <= g_re1_d3d1_overlay_flat_rhw_span &&
        D3D1VerticesMatchBoundsAxis(vertices, unique_vertices, unique_count, &bounds))
    {
      if (out_bounds)
        *out_bounds = bounds;
      if (out_min_alpha)
        *out_min_alpha = min_alpha;
      if (out_max_alpha)
        *out_max_alpha = max_alpha;
      return 1;
    }
  }

  if (ComputeD3D1HTriangleBatchBounds(hvertices, hvertex_count, triangles, triangle_count,
                                      &bounds, unique_vertices, &unique_count))
  {
    const float width = AbsF(bounds.width);
    const float height = AbsF(bounds.height);
    const float extent = width > height ? width : height;
    if (width >= g_re1_d3d1_overlay_h_min_extent &&
        height >= g_re1_d3d1_overlay_h_min_extent &&
        AbsF(bounds.area) >= g_re1_d3d1_overlay_h_min_area &&
        extent <= g_re1_d3d1_overlay_max_extent &&
        AbsF(bounds.max_z - bounds.min_z) <= g_re1_d3d1_overlay_h_flat_z_span)
    {
      if (out_bounds)
        *out_bounds = bounds;
      if (out_min_alpha)
...
static int IsKnownD3D1OverlayTexture(DWORD texture)
{
  if (!texture || texture == 0xFFFFFFFFu)
    return 0;

  LONG count = g_d3d1_overlay_texture_count;
  if (count > (LONG)D3D1_OVERLAY_TEXTURE_CAPACITY)
    count = (LONG)D3D1_OVERLAY_TEXTURE_CAPACITY;
  for (LONG i = 0; i < count; i++)
  {
    if (g_d3d1_overlay_textures[i] == texture)
      return 1;
  }
  return 0;
}

static ZfixTexturePageClass ClassifyD3D1MaskTexture(DWORD texture)
{
  if (!texture || texture == 0xFFFFFFFFu)
  ...
  return ZfixClassifyMaskTexturePage(trace->width, trace->height,
                                     g_re1_d3d1_hires_mask_texture_min_side,
                                     g_re1_d3d1_classic_mask_texture_min_side,
                                     g_re1_d3d1_classic_mask_texture_max_side);
}

static void RememberD3D1OverlayTexture(DWORD texture, const DrawBounds* bounds)
{
  if (!texture || texture == 0xFFFFFFFFu || IsKnownD3D1OverlayTexture(texture))
    return;

  const LONG index = InterlockedIncrement(&g_d3d1_overlay_texture_count) - 1;
  if (index < 0 || index >= (LONG)D3D1_OVERLAY_TEXTURE_CAPACITY)
    return;

  g_d3d1_overlay_textures[index] = texture;
  const LONG learned = InterlockedIncrement(&g_d3d1_overlay_learned);
  if (learned <= 64)
  {
    LogLine("re1 d3d1 overlay-learn #%ld texture=0x%08lX "
            "xy=[%.4f..%.4f %.4f..%.4f] z=[%.6f..%.6f]",
            learned, texture,
            bounds ? bounds->min_x : 0.0f, bounds ? bounds->max_x : 0.0f,
            bounds ? bounds->min_y : 0.0f, bounds ? bounds->max_y : 0.0f,
            bounds ? bounds->min_z : 0.0f, bounds ? bounds->max_z : 0.0f);
...
static int PatchStateValue(BYTE* base, DWORD buffer_size, D3DSTATE_COMPAT* state, DWORD value,
                           D3D1ExecutePatchList* patch_list)
{
  if (!base || !state || !patch_list || state->value == value)
    return 0;

  BYTE* value_ptr = (BYTE*)&state->value;
  if (value_ptr < base || value_ptr + sizeof(DWORD) > base + buffer_size)
    return 0;

  const DWORD offset = (DWORD)(value_ptr - base);
  for (DWORD i = 0; i < patch_list->count; i++)
  {
    if (patch_list->patches[i].value_offset == offset)
    {
      state->value = value;
      return 1;
    }
  ...
  if (patch_list->count >= D3D1_MAX_STATE_PATCHES)
    return 0;

  patch_list->patches[patch_list->count].value_offset = offset;
  patch_list->patches[patch_list->count].original_value = state->value;
  patch_list->count++;
  state->value = value;
  return 1;
}
                                                 ...
                                                 D3D1ExecutePatchList* patch_list)
{
  ...
  if (!g_re1_d3d1_execute_shadow_clip || !execute_buffer || !patch_list)
    return 0;
  ...
  DWORD d3d1_vertex_count = 0;
  const D3DTLVERTEX_COMPAT* d3d1_vertices =
    GetD3D1TLVertices(&exec_data, &desc, &d3d1_vertex_count);
    ...
    while (ip + sizeof(D3DINSTRUCTION_COMPAT) <= end)
    {
          ...
          instr->size >= sizeof(D3DSTATE_COMPAT))
      {
        ...
        for (DWORD i = 0; i < count; i++)
        {
          ...
          else if (state->state == D3DRENDERSTATE_TEXTUREHANDLE)
            last_texture = state->value;
          else if (state->state == D3DRENDERSTATE_ZENABLE)
          {
            // Preserve the renderer's 2D alpha path; masks and effects rely on draw order.
            last_z_enable = state;
          }
               ...
               instr->size >= sizeof(D3DTRIANGLE_COMPAT))
      {
        ...
        if (alpha_active)
        {
          ...
          if (instr->count > 0 && last_z_enable && last_z_enable->value)
          {
            DrawBounds overlay_bounds;
            memset(&overlay_bounds, 0, sizeof(overlay_bounds));
            DWORD overlay_min_alpha = 255u;
            DWORD overlay_max_alpha = 0u;
            const char* skip_reason = NULL;
            int skip_overlay = 0;
            const int exact_d3d1_triangles = instr->size == sizeof(D3DTRIANGLE_COMPAT);
            const ZfixTexturePageClass mask_texture =
              exact_d3d1_triangles && instr->count == 2 ?
              ClassifyD3D1MaskTexture(last_texture) : ZFIX_TEXTURE_PAGE_NONE;
            if (IsKnownD3D1OverlayTexture(last_texture))
            {
              skip_overlay = 1;
              skip_reason = "learned";
            }
            else if (mask_texture != ZFIX_TEXTURE_PAGE_NONE)
            {
              skip_overlay = 1;
              skip_reason = mask_texture == ZFIX_TEXTURE_PAGE_HIRES ?
                            "hires-texture" : "classic-texture";
            }
            else if (exact_d3d1_triangles &&
                     IsLikelyD3D1ScreenOverlayAlpha(d3d1_vertices, d3d1_vertex_count,
                                                    NULL, 0,
                                                    (const D3DTRIANGLE_COMPAT*)ip, instr->count,
                                                    &overlay_bounds, &overlay_min_alpha,
                                                    &overlay_max_alpha))
            {
              skip_overlay = 1;
              skip_reason = "shape";
            ...
            if (skip_overlay)
            {
              const LONG skipped = InterlockedIncrement(&g_d3d1_overlay_skipped);
              const LONG logged = InterlockedIncrement(&g_d3d1_overlay_logged);
              if (logged <= 64)
              {
                LogLine("re1 d3d1 overlay-skip #%ld reason=%s texture=0x%08lX tris=%lu alpha=%lu..%lu "
                        "xy=[%.2f..%.2f %.2f..%.2f] z=[%.6f..%.6f] rhw=[%.8f..%.8f]",
                        skipped, skip_reason ? skip_reason : "?",
                        last_texture, instr->count, overlay_min_alpha, overlay_max_alpha,
                        overlay_bounds.min_x, overlay_bounds.max_x,
                        overlay_bounds.min_y, overlay_bounds.max_y,
                        overlay_bounds.min_z, overlay_bounds.max_z,
                        overlay_bounds.min_rhw, overlay_bounds.max_rhw);
              }
            }
            else
            {
              patches += PatchStateValue(mutable_base, buffer_size, last_z_write, 0, patch_list);
              if (last_z_func && last_z_func->value == D3DCMP_ALWAYS)
                patches += PatchStateValue(mutable_base, buffer_size, last_z_func,
                                           D3DCMP_LESSEQUAL, patch_list);
            }
          }
        }
      }
...
static void LearnD3D1OverlayTexturesAfterExecute(void* execute_buffer)
{
  if (!g_re1_d3d1_skip_screen_overlay_alpha || !execute_buffer)
    return;
  ...
  DWORD hvertex_count = 0;
  const D3DHVERTEX_COMPAT* hvertices = GetD3D1HVertices(&exec_data, &desc, &hvertex_count);
  if (!hvertices || hvertex_count == 0)
  {
    UnlockExecuteBufferData(execute_buffer);
    return;
  }

  const BYTE* base = (const BYTE*)desc.data;
  const DWORD buffer_size = desc.buffer_size;
  if (exec_data.instruction_offset < buffer_size &&
      exec_data.instruction_length <= buffer_size - exec_data.instruction_offset)
  {
    const BYTE* ip = base + exec_data.instruction_offset;
    const BYTE* end = ip + exec_data.instruction_length;
    int alpha_active = 0;
    int z_enabled = 0;
    DWORD texture = 0;
    ...
    while (ip + sizeof(D3DINSTRUCTION_COMPAT) <= end)
    {
      const D3DINSTRUCTION_COMPAT* instr = (const D3DINSTRUCTION_COMPAT*)ip;
      ip += sizeof(D3DINSTRUCTION_COMPAT);
      if (instr->opcode == D3DOP_EXIT)
        break;
      if (instr->size == 0)
      ...
      if (instr->opcode == D3DOP_STATERENDER &&
          instr->size >= sizeof(D3DSTATE_COMPAT))
      {
        const D3DSTATE_COMPAT* states = (const D3DSTATE_COMPAT*)ip;
        for (DWORD i = 0; i < instr->count; i++)
        {
          const D3DSTATE_COMPAT* state = &states[i];
          if ((const BYTE*)(state + 1) > end)
            break;
          if (state->state == D3DRENDERSTATE_ALPHABLENDENABLE)
            alpha_active = state->value != 0;
          else if (state->state == D3DRENDERSTATE_ZENABLE)
            z_enabled = state->value != 0;
          else if (state->state == D3DRENDERSTATE_TEXTUREHANDLE)
            texture = state->value;
        }
      }
               ...
               instr->size >= sizeof(D3DTRIANGLE_COMPAT) &&
               alpha_active && z_enabled && instr->count == 2)
      {
        DrawBounds bounds;
        DWORD min_alpha = 0;
        DWORD max_alpha = 0;
        if (IsLikelyD3D1ScreenOverlayAlpha(NULL, 0, hvertices, hvertex_count,
                                           (const D3DTRIANGLE_COMPAT*)ip, instr->count,
                                           &bounds, &min_alpha, &max_alpha))
        {
          RememberD3D1OverlayTexture(texture, &bounds);
        }
        else
        {
          const LONG logged = InterlockedIncrement(&g_d3d1_overlay_probe_logged);
          if (logged <= 64)
          {
            WORD unique_vertices[12];
            DWORD unique_count = 0;
            if (ComputeD3D1HTriangleBatchBounds(hvertices, hvertex_count,
                                                (const D3DTRIANGLE_COMPAT*)ip, instr->count,
                                                &bounds, unique_vertices, &unique_count))
            {
              LogLine("re1 d3d1 overlay-probe miss #%ld texture=0x%08lX hverts=%lu unique=%lu "
                      "xy=[%.4f..%.4f %.4f..%.4f] z=[%.6f..%.6f] area=%.6f",
                      logged, texture, hvertex_count, unique_count,
                      bounds.min_x, bounds.max_x, bounds.min_y, bounds.max_y,
                      bounds.min_z, bounds.max_z, bounds.area);
            }
            else
            {
              LogLine("re1 d3d1 overlay-probe miss #%ld texture=0x%08lX hverts=%lu no-bounds",
                      logged, texture, hvertex_count);
            }
          }
        }
      }
...
static void RestoreD3D1ExecuteBufferPatches(void* execute_buffer,
                                            const D3D1ExecutePatchList* patch_list)
{
  if (!execute_buffer || !patch_list || patch_list->count == 0)
    return;
  ...
  for (DWORD i = 0; i < patch_list->count; i++)
  {
    const DWORD offset = patch_list->patches[i].value_offset;
    if (desc.buffer_size >= sizeof(DWORD) && offset <= desc.buffer_size - sizeof(DWORD))
    {
      DWORD* value = (DWORD*)(mutable_base + offset);
      *value = patch_list->patches[i].original_value;
    }
  }
...
static HRESULT STDMETHODCALLTYPE Hook_D3DTexture_GetHandle(void* self, void* device, DWORD* handle)
{
  D3DTextureGetHandleProc orig = (D3DTextureGetHandleProc)GetOriginal(*(void***)self, 4);
  if (!orig)
    return E_FAIL;
                                                        ...
                                                        void* viewport, DWORD flags)
{
  ...
  D3D1ExecutePatchList patch_list;
  const DWORD patches =
    PatchD3D1ExecuteBufferForShadowClip(execute_buffer, &tris, &alpha_tris, &patch_list);
  ...
  HRESULT hr = orig(self, execute_buffer, viewport, flags);
  if (SUCCEEDED(hr))
    LearnD3D1OverlayTexturesAfterExecute(execute_buffer);
  RestoreD3D1ExecuteBufferPatches(execute_buffer, &patch_list);
  return hr;
}
...
BOOL WINAPI DllMain(HINSTANCE instance, DWORD reason, LPVOID reserved)
{
  ...
  if (reason == DLL_PROCESS_ATTACH)
  {
    ...
    LogLine("re1_zfix loaded log=%d diagnostics=%d model_callsiteRequired=%d range=0x%08lX..0x%08lX",
            g_log_enabled, g_diagnostics, g_require_model_callsite,
            g_model_callsite_min, g_model_callsite_max);
